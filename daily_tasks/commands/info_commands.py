...
@ck.command
@ck.option('-p', '--export-path',
           type=ck.Path(exists=True,
                        dir_okay=True,
                        resolve_path=True),
           required=True)
def export_tasks(export_path: str):
    """
    Export your tasks, if you use dailytasks less than 3.0.0 version.
    """
...
@ck.command
@ck.option('-a', '--add-tasks',
           is_flag=True,
           required=False,
           help="To add tasks from a json file.")
@ck.option('-f', '--file-name',
           required=False,
           help="Name of the json file where tasks you want add are.")
@ck.option('-d', '--description-key',
           required=False,
           help="Key that contains the descriptions for your tasks.")
@ck.option('-p', '--import-path',
           type=ck.Path(exists=True, dir_okay=True,
                        resolve_path=True),
           required=True,
           help=f"""Directory/Folder where your {utilities.EXPORTED_TASKS_FILE} is,
           Or where your json file with tasks that you want add is
           (to pass this json file use -a flag).""")
@ck.pass_context
def import_tasks(ctx, import_path: str,
                 file_name: str, description_key: str,
    ...
    if add_tasks is False:
        ...
        # !NOTA: Agregar código para importar las sub-tareas.
    else:
        with open(join(import_path, f'{file_name}.json'), 'r', encoding='utf-8') as new_tasks_file:
            new_tasks: list[dict] = json.load(new_tasks_file)
